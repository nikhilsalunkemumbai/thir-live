...
def fmt_commands(commands, max_cmds=20, max_len=90):
    """
    Render a list of commands as a fenced code block.

    Each command goes on its own line. Long strings are truncated at
    max_len chars with ' …' appended so the report stays readable.
    Returns a ready-to-embed Markdown string (fenced block + blank line).
    """
    if not commands:
        return "_none recorded_\n"
    shown = commands[:max_cmds]
    lines = []
    for cmd in shown:
        s = str(cmd).strip()
        if len(s) > max_len:
            s = s[:max_len] + " …"
        lines.append(s)
    remainder = len(commands) - len(shown)
    block = "```\n" + "\n".join(lines) + "\n```\n"
    if remainder > 0:
        block += f"_… and {remainder} more command(s)_\n"
    return block
...
def build_report(ir, threats, fp, stats):
    ...
    if not clean_cases:
        ...
        for cc in clean_cases:
            ...
            # Commands — rendered as a fenced block, one per line
            if commands:
                ln(f"**Commands Executed ({len(commands)}):**")
                ln()
                ln(fmt_commands(commands))
    ...
    ln("| Tool | Role | Status |")
    ln("|---|---|---|")
    ln(f"| Tool 05 | Network Monitor (port 2222)   | {'✅' if hstatus in ('HEALTHY','UP') else '❌'} {hstatus} |")
