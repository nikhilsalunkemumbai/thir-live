...
def build_report(ir, threats, fp, stats, malware):
    ...
    ln(f"| Tool 26 | Incident Timeline Generator | {'✅' if total_cases > 0 else '⚠️'} {total_cases} session(s) |")
    ln(f"| Tool 27 | Threat Intel Feeder         | {'✅' if total_ips > 0 else '⚠️'} {total_ips} IP(s) enriched |")
    ...
    if malware is not None:
        malware_status = f"✅ {malware_total} file(s) analyzed" if malware_total > 0 else "✅ No downloads this shift"
    else:
        malware_status = "⚠️ malware_report.json not found — check pipeline order"
    ln(f"| Tool 31 | Malware Analyzer            | {malware_status} |")
...
def main():
    ...
    report = build_report(ir, threats, fp, stats, malware)
